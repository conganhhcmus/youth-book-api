...
export const exportAnalyticsDetail = async (userId: string, type: number): Promise<Buffer> => {
    ...
    worksheet.columns = [
        ...
        { header: 'Tên chương', key: 'chapterName', width: 50, style: defaultStyle },
        { header: 'Xem lúc', key: 'createTime', width: 30, style: { ...defaultStyle, numFmt: 'HH:MM dd/mm/yyyy' } },
    ];

    result.forEach((data, index) => {
        worksheet.addRow({ ...data, index: index + 1 });
    });
